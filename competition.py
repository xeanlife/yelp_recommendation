...
yelp_train_transformed_X_df_nona = yelp_train_transformed_X_df.copy()
yelp_train_transformed_X_df_nona[1] = yelp_train_transformed_X_df_nona[1].fillna(yelp_train_transformed_X_df_nona[1].mean()) # fill user year NA
yelp_train_transformed_X_df_nona[7] = yelp_train_transformed_X_df_nona[7].fillna(yelp_train_transformed_X_df_nona[7].mean()) # fill user rating NA
yelp_train_transformed_X_df_nona[21] = yelp_train_transformed_X_df_nona[21].fillna(yelp_train_transformed_X_df_nona[21].mean()) # fill business rating NA
yelp_train_transformed_X_df_nona[60] = yelp_train_transformed_X_df_nona[60].fillna(yelp_train_transformed_X_df_nona[7].mean()) # fill friends rating NA
yelp_train_transformed_X_df_nona = yelp_train_transformed_X_df_nona.fillna(0) # fill rest of the NA with 0



enet_param = {'alpha':[0.0001, 0.0003, 0.0005, 0.0007], 'l1_ratio':[0.3, 0.4, 0.5]} # generate parameters to test
enet_grid = GridSearchCV(enet, enet_param, scoring='neg_root_mean_squared_error', verbose=3, cv=5, n_jobs=2)
enet_grid.fit(yelp_train_transformed_X_df_nona, yelp_train_transformed_y_df) # fit model
enet_grid.best_params_ # {'alpha': 0.0001, 'l1_ratio': 0.4}
enet_grid.best_score_

ridge_param = {'alpha':[0.0001, 0.001, 0.01, 0.1, 1, 10, 100]} # generate parameters to test
ridge_grid = GridSearchCV(ridge, ridge_param, scoring='neg_root_mean_squared_error', verbose=3, cv=5, n_jobs=2)
ridge_grid.fit(yelp_train_transformed_X_df_nona, yelp_train_transformed_y_df) # fit model
ridge_grid.best_params_
ridge_grid.best_score_

lasso_param = {'alpha':[0.0001, 0.0003, 0.0005, 0.0007]} # generate parameters to test
lasso_grid = GridSearchCV(lasso, lasso_param, scoring='neg_root_mean_squared_error', verbose=3, cv=5, n_jobs=2)
lasso_grid.fit(yelp_train_transformed_X_df_nona, yelp_train_transformed_y_df) # fit model
lasso_grid.best_params_ # {'alpha': 0.0001}
lasso_grid.best_score_


cat = CatBoostRegressor(n_estimators=700, learning_rate=0.05, depth=10, loss_function='RMSE', silent=True)
enet = make_pipeline(RobustScaler(), ElasticNet(alpha=0.0001, l1_ratio=0.4))
lasso = make_pipeline(RobustScaler(), Lasso(alpha=0.0001, max_iter=1000000))
ridge = make_pipeline(RobustScaler(), Ridge(max_iter=1000000))
stacked = StackingRegressor(regressors=(enet, ridge, lasso), meta_regressor=cat, use_features_in_secondary=True, verbose=2)
stacked_fit = stacked.fit(yelp_train_transformed_X_df_nona, yelp_train_transformed_y_df.values.ravel())


yelp_test_transformed_X_df_nona = yelp_test_transformed_X_df.copy()
yelp_test_transformed_X_df_nona[1] = yelp_test_transformed_X_df_nona[1].fillna(yelp_test_transformed_X_df_nona[1].mean()) # fill user year NA
yelp_test_transformed_X_df_nona[7] = yelp_test_transformed_X_df_nona[7].fillna(yelp_test_transformed_X_df_nona[7].mean()) # fill user rating NA
yelp_test_transformed_X_df_nona[21] = yelp_test_transformed_X_df_nona[21].fillna(yelp_test_transformed_X_df_nona[21].mean()) # fill business rating NA
yelp_test_transformed_X_df_nona[60] = yelp_test_transformed_X_df_nona[60].fillna(yelp_test_transformed_X_df_nona[7].mean()) # fill friends rating NA
yelp_test_transformed_X_df_nona = yelp_test_transformed_X_df_nona.fillna(0) # fill rest of the NA with 0

stacked_pred = stacked_fit.predict(np.array(yelp_test_transformed_X_df_nona))
...
# cat_reg = CatBoostRegressor(n_estimators=700, learning_rate=0.05, depth=10, loss_function='RMSE', silent=True)
...
xgb_tree_best = xgb.XGBRegressor(n_estimators=800, learning_rate=0.05, max_depth=6, colsample_bytree=0.3)
...
yelp_test_result['prediction'] = xgb_predicted
yelp_test_result['prediction'] = cat_predicted
yelp_test_result['prediction'] = stacked_pred
combined_predicted = [np.mean(xgb_cat) for xgb_cat in zip(xgb_predicted, cat_predicted, stacked_pred)]
...
yelp_test_result['prediction'] = combined_predicted # combine results 
